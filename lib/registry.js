...
function vhost (hostname, remove) {
  if (hostname.match(/[^\s]\.localhost$/)) {
    if (remove) {
      var keep = [];
      var hosts = fs.readFileSync("/etc/hosts", "utf8").split("\n");
      for (var h in hosts) {
        if (hosts[h].match(new RegExp("\\s" + hostname + "$")) === null) {
          keep.push(hosts[h]);
        }
      }
      fs.writeFileSync("/etc/hosts", keep.join("\n"));
    } else {
      var hosts = fs.readFileSync("/etc/hosts", "utf8").split("\n");
      var exists = false;
      for (var h in hosts) {
        if (hosts[h].match(new RegExp("\\s" + hostname + "$"))) {
          exists = true;
          break;
        }
      }
      if (exists === false) {
        hosts.push("127.0.0.1 " + hostname + "\n");
